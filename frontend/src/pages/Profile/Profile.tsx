...
export default function Profile() {
  ...
  const getAvatarUrl = useCallback((): string | null => {
    if (customization.customAvatarUrl) return customization.customAvatarUrl
    if (user?.photoURL) return user.photoURL
    return null
  }, [customization.customAvatarUrl, user?.photoURL])

  // Get premade avatar emoji if no photo
  const getAvatarEmoji = useCallback((): string | null => {
    const avatar = AVATARS.find((a) => a.id === customization.avatarId)
    return avatar?.preview || null
  ...
  // Determine Google link status from auth provider
  const isGoogleLinked = user?.photoURL !== null && user?.photoURL !== undefined
      ...
      <div className="profile-content">
          ...
          <div className="profile-avatar-section">
            ...
            <div className="profile-info">
              {isEditing ? (
                ...
                <h2 className="display-name">{user?.displayName || 'Player'}</h2>
              )}
              <span className="email">{user?.email || ''}</span>
              <span className="join-date">Member since {getJoinDate()}</span>
            </div>
          </div>
        ...
        <div className="profile-bottom-grid">
          ...
          <div className="profile-card linked-card">
            ...
            <div className="linked-accounts">
              <div className={`linked-account ${isGoogleLinked ? 'connected' : ''}`}>
        ...
        <div className="profile-card quick-links-card">
          ...
          <div className="quick-links">
            <Link to="/my-decks" className="quick-link">
